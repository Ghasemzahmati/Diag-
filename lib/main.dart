...
class EcuDatabaseEntry {
  ...
  final List<String> actuatorCapabilities;
  ...
  const EcuDatabaseEntry({
    ...
    this.responseHeader, this.services = const [], this.livePids = const [],
    this.actuatorCapabilities = const [], required this.confidence, required this.sourceNote,
  });
...
const rasaEcuDatabase = <EcuDatabaseEntry>[
  EcuDatabaseEntry(
    ...
    services: ['KWP DTC', 'Live Data', 'Actuator Tests'],
    livePids: ['RPM', 'ECT', 'TPS', 'MAP', 'IAT', 'Battery'],
    actuatorCapabilities: ['انژکتورها', 'کویل‌ها', 'پمپ بنزین', 'فن‌ها', 'کنیستر', 'چراغ MIL'],
  ...
  EcuDatabaseEntry(
    ...
    services: ['KWP DTC', 'Live Data', 'Actuator Tests'],
    livePids: ['RPM', 'ECT', 'TPS', 'MAP', 'IAT', 'Battery'],
    actuatorCapabilities: ['انژکتور 1-4', 'پمپ بنزین', 'کویل‌ها', 'فن دور کند/تند', 'کنیستر', 'Idle Actuator', 'چراغ MIL'],
    confidence: EcuConfidence.partial,
  ...
  EcuDatabaseEntry(
    ...
    services: ['OBD-II', 'Live Data', 'Actuator Tests'],
    livePids: ['RPM', 'ECT', 'TPS', 'MAP', 'IAT', 'Vehicle Speed', 'Battery'],
    actuatorCapabilities: ['Fuel Pump', 'Cooling Fan', 'Injectors', 'Ignition'],
    confidence: EcuConfidence.partial,
...
class _RasaDashboardScreenState extends State<RasaDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _configureProtocolTransport() async {
    ...
    switch (_protocolCode) {
      case '6': case '8':
        ...
        await _sendAndWait('ATSH 7DF', timeout: const Duration(seconds: 1));
        break;
      case '7': case '9':
        await _sendAndWait('ATCAF1', timeout: const Duration(seconds: 1));
        await _sendAndWait('ATCFC1', timeout: const Duration(seconds: 1));
  ...
  Future<void> _discoverEcuDatabase() async {
    ...
    try {
      ...
      // Second: if CAN is active, try the common engine diagnostic address.
      // 7E0 is a discovery candidate, NOT a claim that every OEM uses it.
      if (_protocolCode == '6' || _protocolCode == '7' || _protocolCode == '8' || _protocolCode == '9') {
        await _sendAndWait('ATCAF1', timeout: const Duration(seconds: 1));
        await _sendAndWait('ATCFC1', timeout: const Duration(seconds: 1));
        await _sendAndWait('ATSH 7E0', timeout: const Duration(seconds: 1));
        final f190 = await _udsReadDid(0xF190);
        final udsVin = _decodeUdsVin(f190);
        if (udsVin.isNotEmpty) {
          vin = udsVin;
          _vin = udsVin;
          _discoveryTrace.add('UDS 22F190 @ 7E0 => $udsVin');
        } else {
          _discoveryTrace.add('UDS 22F190 @ 7E0 => NO DATA');
        }

        // These are read-only identification probes. The actual DID meaning can be OEM-specific.
        for (final did in const [0xF187, 0xF188, 0xF189, 0xF180, 0xF181]) {
          final value = await _readUdsAsciiDid(did);
          if (value.isNotEmpty) {
            _discoveryTrace.add('UDS 22${did.toRadixString(16).padLeft(4,'0').toUpperCase()} => $value');
            if (did == 0xF187) _hwId = value;
            if (did == 0xF188 || did == 0xF181) _swId = value;
          }
        }
      }
  ...
  Future<void> _runUdsIdentification() async {
    ...
    try{
      ...
      if (_matchedEcu?.requestHeader != null) {
        ...
        await _sendAndWait('ATSH 7E0',timeout:const Duration(seconds:1),logCommand:true);
      }
  ...
  Widget _ecuTab() => ListView(padding: const EdgeInsets.all(14), children: [
        ...
        _info('بانک ECU', _databaseStatus),
        ...
        _info('هدر درخواست', _matchedEcu?.requestHeader ?? _profile.header ?? 'Trace / Auto'),
        _info('VIN', _vin),
        ...
        if (_dbMatches.isNotEmpty) ...[
          const Text('نتایج بانک', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
