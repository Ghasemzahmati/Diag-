...
class OdometerDefinition {
  final String id, ecuFamily, serviceHex, didHex, unit, verification;
  final DiagnosticProtocol protocol;
  final int dataOffset, dataLength;
  final bool bigEndian;
  final double scale, offset;
  const OdometerDefinition({required this.id, required this.protocol, required this.ecuFamily, required this.serviceHex, required this.didHex, required this.dataLength, required this.verification, this.dataOffset=0, this.bigEndian=true, this.scale=1, this.offset=0, this.unit='KM'});
}
...
class _RasaDashboardScreenState extends State<RasaDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Future<String> _sendAndWait(String command, {Duration timeout = const Duration(milliseconds: 1500), bool logCommand = false}) async {
    ...
    try {
      ...
      if (identical(_waiters[key], completer)) _waiters.remove(key);
    }
  }
  ...
  int? _parsePid(String response, String pid) {
    ...
    return int.tryParse(clean.substring(start, start + 2), radix: 16);
  }
  ...
  void _parseResponse(String response) {
    ...
    final pids = <String, void Function(int)> {
      '0C': (a) => _rpm = a,
      '0D': (a) => _speed = a,
      '05': (a) => _coolant = a - 40,
      '11': (a) => _throttle = ((a * 100) / 255).round(),
      '04': (a) => _engineLoad = ((a * 100) / 255).round(),
      '0F': (a) => _iat = a - 40,
      '14': (a) => _o2 = a / 200.0,
    };
    for (final e in pids.entries) {
      final v = _parsePid(response, e.key);
      if (v != null && mounted) setState(() => e.value(v));
    }
  ...
  Future<void> _runActuator(String name) async {
    ...
    if(def.serviceHex.isEmpty || def.identifierHex.isEmpty || def.startPayloadHex.isEmpty || def.stopPayloadHex.isEmpty){
      await showDialog(
        context:context,
        builder:(_)=>AlertDialog(
  ...
  void _toggleDemo(bool enable) {
    ...
    _demoTimer = Timer.periodic(const Duration(milliseconds: 300), (_) {
      if (!mounted) return;
      setState(() {
        _speed = (_speed + r.nextInt(7) - 3).clamp(0, 180);
        _rpm = (_speed * 30 + 850 + r.nextInt(250)).clamp(800, 6500);
        _coolant = 88 + r.nextInt(5);
        _throttle = (_speed / 2.0).round().clamp(0, 100);
        _engineLoad = (15 + _speed / 2.5).round().clamp(15, 95);
        _iat = 30;
  ...
  Widget _dashboard() => ListView(padding: const EdgeInsets.all(14), children: [
        ...
        Row(children: [
          Expanded(child: RasaGauge(label: 'دور موتور', value: '$_rpm', unit: 'RPM', progress: (_rpm / 7000).clamp(0, 1))),
          const SizedBox(width: 10),
          Expanded(child: RasaGauge(label: 'سرعت', value: '$_speed', unit: 'KM/H', progress: (_speed / 240).clamp(0, 1))),
        ]),
        const SizedBox(height: 12),
        Row(children: [
  ...
  Widget _actuatorTab(){
    ...
    return ListView(padding:const EdgeInsets.all(14),children:[
      Card(child:ListTile(leading:Icon(verifiedActuatorDefinitions.isNotEmpty?Icons.verified:Icons.lock_outline,color:verifiedActuatorDefinitions.isNotEmpty?const Color(0xFF00E676):const Color(0xFFFFD600)),title:Text(verifiedActuatorDefinitions.isNotEmpty?'بانک عملگر آماده است':'بانک عملگر ECU هنوز تأیید نشده'),subtitle:Text('ECU: ${_profile.title}'))),
      const SizedBox(height:8),
...
class GaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: c, radius: r), pi * .75, pi * 1.5 * progress.clamp(0, 1), false, fg);
  }
  @override
